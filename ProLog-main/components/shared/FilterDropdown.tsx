...
export const FilterDropdown: React.FC<FilterDropdownProps> = ({ options, selected, onSelect, placeholder, style }) => {
  ...
  return (
    <View style={{ position: 'relative' }}>
      ...
      >
        <Text style={selected ? [styles.dropdownText, { color: Colors.grey[900] }] : styles.dropdownText}>
          {selected || placeholder || 'Select'}
        </Text>
        <MaterialIcons name="arrow-drop-down" size={16} color={Colors.grey[500]} style={{ marginLeft: 8 }} />
      </TouchableOpacity>
      {visible && (
        <TouchableOpacity
